...
import codingames.veryhard.knight.Player0
...
class SolutionTests extends FlatSpec {
  ...
  "A solution" should "return matrix of distances" in {
    val distanceMatrix = Player0.distance((5,1), matrixDimension)
//    Player.printMatrix(matrixDimension, distanceMatrix)
  }

  "A solution" should "compare two matrices" in {
    val distanceMatrixPrev = Player0.distance((5,1), matrixDimension)
    val distanceMatrixNext = Player0.distance((0,0), matrixDimension)
...
//    Player.printMatrix(matrixDimension, distanceMatrixNext)
    val processedMatrix: Array[Float] = Player0.delta[Float](distanceMatrixPrev, distanceMatrixNext, FloatMapFunction).toArray
    Console.err.println("\n")
...
//    Player.printMatrix(matrixDimension, processedMatrix)
  ...
  "A solution" should "return boolean matrix" in {
    val distanceMatrixPrev = Player0.distance((5,1), matrixDimension)
    val distanceMatrixNext = Player0.distance((1,5), matrixDimension)
    Console.err.print("PREV")
//    Player.printMatrix(matrixDimension, distanceMatrixPrev)
    Console.err.println("\n")
    Console.err.print("NEXT")
...
//    Player.printMatrix(matrixDimension, processedMatrix)
    ...
    val validityMatrix = Player0.cut(processedMatrix, cutMatrix, "COLDER", booleanMapFunction).toArray
//    Player.printMatrix(matrixDimension, validityMatrix)
  ...
  "A solution" should "map boolean matrix" in {
    val point = (17, 31)
    val distanceMatrixPrev = Player0.distance(point, matrixDimension)
    cutMatrix(point) = false
    val distanceMatrixNext = Player0.distance((10,4), matrixDimension)
...
//    Player.printMatrix(matrixDimension, validityMatrix)
    ...
    val resultMatrix = Player0.delta(validityMatrix, cutMatrix, booleanMapFunction).toArray
