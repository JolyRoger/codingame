...
//  def turnTo(id: Int, str: String) =
  ...
  def closestToHQ(p: (Int, Int), items: List[(Int, Int)], dist: Array[Int], board: Board) =
    if (items.isEmpty) (p._1, p._2) else items.minBy(item => dist(board.toNumber((item._1, item._2))))
