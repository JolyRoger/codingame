...
//----------------------------------------------------------------------------------------------------------------------
  ...
  def calcBfsMap(mine: List[Robot], enemy: List[Tile],
                 tiles: Array[Tile], bfsResMap: Map[Int, BfsResult],
                 unitMap: Set[Int], available: Reachable): BfsMap = {
    def bfsToUnit(bfs: BfsResult, enemy: Tile): Option[Array[Int]] = {
      Some { enemy.freeSquaresAsNum(unitMap, tiles)
    ...
    mine.map(me => (me.id, bfsToUnits(me, enemy, available))).toMap
  }
  ...
  for (_ <- LazyList.from(0).takeWhile(_ < 201)) {
    ...
    val withScraps = tiles.filter(tile => tile.scrapAmount > 0 && !tile.hasRecycler).toList
    ...
    val bfsResMap = myRobots.map(robot => (robot.id, bfs(robot, tiles, foeIndices, alwaysTrue))).toMap
    val bfsData = calcBfsMap(myRobots, withScraps, tiles, bfsResMap, foeIndices, alwaysTrue)
